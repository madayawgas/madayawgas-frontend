...
export default function TruckModal({
  ...
  const handleInputChange = (e) => {
    ...
    if (
      ["currentOdometer", "lastPMOdometer", "yearModel", "capacity"].includes(
        name
      )
    ) {
  ...
  const validate = () => {
    ...
    if (
      (formData.status === "AVAILABLE" || formData.status === "IN_USE") &&
  ...
  const submitSave = () => {
    ...
    const finalData = {
      ...formData,
      driverName: matchedDriver ? matchedDriver.label : "Unassigned",
  ...
  const editFooter = (
    <div className="flex justify-end gap-3">
      ...
      <Button variant="primary" onClick={submitSave}>
        {isAdding ? "ADD TRUCK" : "SAVE"}
      </Button>
    </div>
  );
  ...
  const viewFooter = (
    <div className="flex justify-center gap-3">
      ...
      <Button variant="danger" onClick={() => onDeleteClick(displayTruck)}>
        DELETE
      </Button>
      <Button variant="secondary" onClick={onClose}>
        CLOSE
    ...
    >
      {isEditing || isAdding ? (
        ...
        <div className="text-left space-y-4 text-gray-800 py-2">
          <p>
            Assigned Driver:{" "}
            <span className="font-medium">{displayTruck.driverName}</span>
          </p>
          <p>
            Truck Model:{" "}
            <span className="font-medium">
              {displayTruck.yearModel} {displayTruck.model}
            </span>
          </p>
          <p>
            Capacity:{" "}
            <span className="font-medium">{displayTruck.capacity} L/T</span>
          </p>
          <p>
            Current Odometer:{" "}
            <span className="font-medium">
          ...
          <p>
            Last PM Odometer:{" "}
            <span className="font-medium">
              {displayTruck.lastPMOdometer?.toLocaleString()} KM
            </span>
          </p>
